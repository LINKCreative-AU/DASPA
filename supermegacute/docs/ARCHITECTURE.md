...
- **Affiliate IDs live only in config.** Products reference `{merchant, merchantRef}`; `SMC.affiliateLink()` builds the outbound URL at render time, appends UTM defaults and applies `rel="sponsored nofollow noopener"`.
...
- This prototype lives in `supermegacute/` on a branch of the DASPA repo purely for review convenience; production will be its own repo + Vercel project on supermegacute.com (same static + serverless + Supabase stack the team already runs).
...
- `merchantRef` values and affiliate IDs are placeholders until network approvals land.
