      ...
      {options.map((o) => (
        <button
          ...
          className={`rounded-full border px-3 py-1.5 text-[13px] font-semibold transition ${
            value === o
              ? "border-advance bg-advance text-white"
...
export function ContactForm({
  ...
  if (sent) {
    return (
      <div className="rounded-3xl border border-ink/10 bg-white p-8">
        <p className="text-sm font-semibold uppercase tracking-wider text-advance">
        ...
        {guide ? (
          <>
            ...
            <a
              ...
              className="btn btn-advance mt-5"
            >
              Download the guide
            </a>
          </>
  ...
  return (
    <form onSubmit={submit} className="rounded-3xl border border-ink/10 bg-white p-6 sm:p-7">
      <div className="space-y-4">
        <div>
          <p className="text-sm font-bold text-ink">
            {guide ? "What's driving the interest?" : "What brings you to LINK Advance?"}
          </p>
          <Chips
            options={REASONS}
            value={form.reason}
        ...
        <div className="border-t border-ink/10 pt-4">
              ...
              <>
                ...
                <label className="block">
                  ...
                  <select
                    ...
                    className="w-full rounded-lg border border-line bg-white px-4 py-2.5 text-sm focus:border-advance focus:outline-none"
                  >
          ...
          {showMsg || form.message ? (
            <label className="mt-4 block">
              ...
              <textarea
                ...
                className="w-full rounded-lg border border-line px-4 py-2.5 text-sm focus:border-advance focus:outline-none"
              />
            </label>
          ) : (
            <button
          ...
          <label className="mt-3 flex items-start gap-2.5 text-xs text-ink/60">
            <input
              ...
              className="mt-0.5 h-4 w-4 accent-advance"
            />
      ...
      <button type="submit" disabled={busy} className="btn btn-advance mt-5 w-full sm:w-auto">
        {busy
          ? "Sending…"
          : guide
