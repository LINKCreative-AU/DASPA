...
7. **Korean and Taiwanese landing pages**: BUILT, and deliberately `noindex`. `/ko` and
   `/zh-tw` exist with full copy, FAQPage schema, CJK font fallbacks and an hreflang cluster
   wired to the English homepage in both directions. They are held back because this is a
   registered tax agent's site making statements about Australian tax law, so **a native
   reader must sign the copy off before the noindex comes off**, one page at a time. When
   they go live they also need a visible language switcher, which is not built yet.
   The original note, for context: the surprise in the data was South Korea 700/mo and
