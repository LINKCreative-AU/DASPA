...
// Ported from the Advisors build; keywords tuned for wealth topics.
  ...
  return (
    <ol className={`mt-12 grid gap-8 ${colClass}`}>
      {steps.map((s, i) => (
        <li key={s.title}>
          <span
            className={`font-display text-3xl font-semibold ${dark ? "text-wealth-bright/70" : "text-wealth/40"}`}
          >
      ...
      {items.map((c) => (
        <li key={c} className={`flex gap-3 ${dark ? "text-white/80" : "text-ink/75"}`}>
          <svg
            ...
            fill="none"
            aria-hidden
            className={`mt-1 shrink-0 ${dark ? "text-wealth-bright" : "text-wealth"}`}
          >
  ...
  return (
    <section className="border-y border-ink/10 bg-neutral-50">
      <div className="container-x flex flex-wrap items-center justify-between gap-4 py-5 text-sm font-semibold text-ink/75">
        ...
        <span>Licensed financial advisers</span>
        <span>30+ years experience</span>
        <span>$660 workshops, Value Guarantee</span>
